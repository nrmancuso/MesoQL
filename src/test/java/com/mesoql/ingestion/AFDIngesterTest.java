...
class AFDIngesterTest {
    ...
    @Test
    void chunkShortText() {
        List<String> chunks = AFDIngester.chunk("hello world", 512, 64);
    ...
    void chunkLongText() {
        ...
        List<String> chunks = AFDIngester.chunk(sb.toString().trim(), 512, 64);
        assertTrue(chunks.size() > 1);
        // chunks should overlap
        String[] firstWords = chunks.get(0).split("\\s+");
        String[] secondWords = chunks.get(1).split("\\s+");
